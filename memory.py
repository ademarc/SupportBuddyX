...
def delete_user_memory(user_id):
    with LockManager(user_id):
        try:
            # Delete the user's memory in MongoDB
            memories.delete_one({'user_id': user_id})
            logger.info(f'Successfully deleted memory for user {user_id}')
        except Exception as e:
            logger.error(f'Failed to delete memory for user {user_id}.', exc_info=True)
            raise e
